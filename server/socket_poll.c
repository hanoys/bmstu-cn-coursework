...
int send_headers(char *path, int clientfd) {
    char status[] = OK_STR, connection[] = "Connection: close";
    char *len = calloc(HEADER_LEN, sizeof(char)), *type = calloc(HEADER_LEN, sizeof(char));
    char *res_str = "/0";
...
void process_head_req(char *path, int clientfd) { send_headers(path, clientfd); }
